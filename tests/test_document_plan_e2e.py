...
from document_plan import (
    DocumentPlan,
    document_plan_to_markdown,
    update_linked_chart_value,
)
...
def test_linked_edit_reaches_final_hwpx_with_recomputed_metric_and_chart(tmp_path):
    ...
    # The edit is local to the returned plan and propagates only through explicit links.
    ...
    semantic_markdown = document_plan_to_markdown(edited)
    assert "58.3%" in semantic_markdown
    assert "| 2025 | 1000 |" in semantic_markdown
    assert "불용액: 2400, 1000" in semantic_markdown
    ...
    chart_bytes = (tmp_path / "chart_1.png").read_bytes()
    markdown_path = tmp_path / "edited_input.md"
    output_path = tmp_path / "track_c_edited_e2e.hwpx"
    markdown_path.write_text(rendered_markdown, encoding="utf-8")

    result = generate_hwpx(markdown_path, output_path, preset=edited.preset, timeout=120)
    assert result.ok, result.diagnostic
    ...
    with zipfile.ZipFile(output_path) as archive:
        ...
        assert "불용액 개선 수정 보고서" in section
        assert "불용액 감소" in section
